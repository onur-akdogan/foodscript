                ...
                <table class="table datatable-responsive" id="example">
                    ...
                    @foreach ($comments as $row )
                        <tr>
                            ...
                            <td>{{$row->details}}</td>
                            <td> @if ($row->status == 1)
                                    <form action="{{ route('active.comments', $row->id) }}" method="post">
                                        @csrf
                            ...
                            <td><a href="{{route('delete.comments',$row->id)}}"
                                   onclick="return confirm('Silmek istediğinizden emin misiniz ?')"
                                   class="dropdown-item"><i class="icon-trash"></i>Sil</a></td>


                            <td><a href="{{route('open.comments',$row->posts_id)}}"
                                   class="dropdown-item"><i class="icon-link"></i>Tarife Git</a></td>
